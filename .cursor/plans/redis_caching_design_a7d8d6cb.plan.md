...
todos:
  - id: infra
    content: Add Redis client/package, REDIS_URL, compose service, fail-open
    status: completed
  - id: settings-cache
    content: Cache GetApplicationSettings + public auth config; invalidate on settings writes
    status: completed
  - id: authz-cache
    content: Cache IsSignInBlocked + role permissions; invalidate on user/role writes
    status: completed
  - id: exams-cache
    content: Cache published exam catalogs/by-id; invalidate on exam/content writes
    status: completed
isProject: false
---
